...
INTERPRETER_PREFIX = "/ai " # Define the prefix for AI commands
...
class TerminalApp:
    def __init__(self, master):
        ...
        self.interpreter_api_base = self.config.get('interpreter_api_base', None) # Default to None (let interpreter handle)
        # --- End Open Interpreter Config ---
    ...
    def prompt_for_ai_model(self):
        """Prompts the user to enter the model for the open interpreter."""
        model = simpledialog.askstring("AI Model",
                                       "Enter the local model name (e.g., ollama/mistral, gpt-4):",
                                       initialvalue=self.interpreter_model,
                                       parent=self.master)
        if model: # User didn't cancel
            # Basic validation (optional): check if model string looks reasonable
            if '/' in model or ':' in model or model.isalnum(): # Very basic check
                ...
                messagebox.showerror("Invalid Model", f"Model name '{model}' seems invalid.", parent=self.master)

    def prompt_for_api_base(self):
        """Prompts the user to enter the API base URL for the AI model."""
        api_base = simpledialog.askstring("AI API Base URL",
                                          "Enter the API base URL (e.g., http://localhost:1234/v1) or leave blank to unset:",
                                          initialvalue=self.interpreter_api_base or "", # Show empty if None
                                          parent=self.master)

        if api_base is not None: # User didn't cancel (empty string is a valid input to unset)
            if api_base.strip(): # User entered something
    ...
    def run_interpreter_command(self, prompt, output_widget):
        ...
        try:
            # You might want to reuse the instance if config hasn't changed,
            # but creating a new one for each command is simpler for now.
            self.interpreter_instance = interpreter
            self.interpreter_instance.llm.model = self.interpreter_model
            self.interpreter_instance.llm.api_base = self.interpreter_api_base # Use configured API base (can be None)
            self.interpreter_instance.auto_run = True # Automatically run generated code
            self.interpreter_instance.loop = False # Prevent conversational looping within one command
